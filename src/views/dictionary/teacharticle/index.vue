...
<script lang="ts">
  ...
  //api
  import { getList,del,upLock } from '/@/api/dictionary/teacharticle';
  export default defineComponent({
    ...
    setup(_, { emit }) {
     ...
     async function handleDelete(record: Recordable) {
        const result =await del({id:record.id})
        if(result){
          reload();
        }
      }
     ...
     async function onUpStatus(id,checked) {
        try {
            createMessage.loading({ content: '更新状态中...', key:"upLock",duration:0});
            const resultdata = await upLock({id:id,status:checked});
            if(resultdata){
              createMessage.success({ content: '更新成功！', key:"upLock", duration: 2 });
            }else if(resultdata==0){
              createMessage.success({ content: '已更新！', key:"upLock", duration: 2 });
            }
          } catch {
            createMessage.destroy("upLock");
          }
      }
      return {
        registerModal,
